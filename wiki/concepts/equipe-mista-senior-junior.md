...
# Equipe Mista Sênior-Júnior

Tese de [[wiki/entities/fabio-akita]]: nem uma equipe só de sêniors, nem uma equipe só de júniors funciona bem para a maioria absoluta dos casos. O time ideal combina os dois níveis, porque cada um resolve um problema que o outro não resolve sozinho.

## A metáfora do pedreiro e do mestre de obras

Um mestre de obras (sênior) forçado a erguer parede de tijolo o dia inteiro (tarefa rotineira/mundana) é improdutivo nessa tarefa — não porque não saiba fazer, mas porque não é o trabalho que otimiza seu tempo. O pedreiro (júnior) faz isso mais rápido, porque é o trabalho do dia a dia dele. Por outro lado, perguntar a um pedreiro qual fundação é mais adequada para um terreno específico expõe a falta de experiência acumulada — o mestre de obras já viu esse problema várias vezes e filtra rápido entre as opções.

## Por que um time só de sêniors falha

Um cliente que pede "5 sêniors" geralmente não precisa de 5 sêniors — precisa de 1 sênior, 1 pleno e 3 júniors. Colocar só sênior em tarefas rotineiras (tela simples, CSS responsivo) gera improdutividade e frustração, e desperdiça a capacidade de decisão do sênior em trabalho que não a exige.

## Por que um time só de júniors falha
...
- [[wiki/concepts/mentoria-tecnica]] — a prática que faz o time misto funcionar de fato
